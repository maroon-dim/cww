...
test('renderer handles pointer input, dialog visibility, resize and reduced motion',()=>{
  ...
  const makeHost=(width,height,open)=>({clientWidth:width,clientHeight:height,open,prepend(canvas){this.canvas=canvas;canvas.host=this;}});
  const map=makeHost(1440,900,true),dialog=makeHost(920,900,false);
  const observers=[];
  const document={hidden:false,documentElement:{addEventListener(){}},addEventListener:(name,fn)=>docEvents[name]=fn,
  ...
  assert.ok(vm.runInContext('fields.every(f=>f.rain.length<=90 && f.points.length<=240)',context));
  events.pointermove({clientX:400,clientY:300});tick(132);
  dialog.open=true;tick(148);assert.ok(contexts[1].glyphs>0,'open dialog renders');
